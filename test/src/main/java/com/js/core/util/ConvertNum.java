...
public class ConvertNum {
    ...
    private static final org.slf4j.Logger logger = LoggerFactory.getLogger(ConvertNum.class);
    /**
     * 把金额阿拉伯数字转换为汉字表示，小数点后四舍五入保留两位 还有一种方法可以在转换的过程中不考虑连续0的情况，然后对最终的结果进行一次遍历合并连续的零
     */
    ...
    public static String NumToChinese(double num) {
        if (num > 99999999999999.99 || num < -99999999999999.99) {
            logger.error("参数值超出允许范围 (-99999999999999.99 ～ 99999999999999.99)！");
            return "";
        }
    ...
    public static String partConvert(int partNum) {
        if (partNum < 0 || partNum > 10000) {
            logger.error("参数必须是大于等于0或小于10000的整数");
            return "";
        }
